...
### Changelog (v2.1 feedback pass 5)
...
- **Rule 2: Conflict Detection**:
  ...
  - If the account has **$\ge 1$ saved rides**: A clear modal interrupts the login redirect:
    ...
    >
    > - **[Add 4 coasters to the bottom of my account]** _(Guest-only coasters join at the bottom; your order is untouched)_
...
- A 10-second undo toast appears (_"Added 3 coasters to your ranking · [Undo]"_); undo removes the appended rides via the existing `useRemoveRide` mutation (N ≤ 100, looped).
...
- **v1.1 Fast-Follow**: Add a compact `"I've Ridden This"` pill on `/coasters/:slug`.
  - Clicking it adds the coaster to the active guest selection set and shows a toast:
    > _Added to your rides · [Mark more on board] or [Rank my rides (N)]_
  ...
  options: {
    data: {
      ...
      pending_guest_rides: guestState
        ? {
            ids: guestState.orderedIds,
            started_at: new Date(guestState.createdAt).toISOString(),
          }
        : undefined,
    },
  },
...
Companion table — same RLS shape as `import_events`: users append their own rows via the RPCs (`insert` with `user_id = auth.uid()` check); reads are admin-only.
...
| :---------------------------------------- | :------------------------------------------ | :------------------------------------------------------------------------------------------ |
| `guest_promotions.kind = 'materialize'`   | `materialize_guest_rides` RPC               | ride_count (rows newly ranked), duration_ms (server-computed from `started_at`), created_at |
