...
- **Exact error matching.** pgTAP `throws_ok` compares errcode and message exactly (no `LIKE`), so expected messages must be copied verbatim — e.g. PG12+ check text is `new row for relation "t" violates check constraint "c"`.
...
pg_prove -h localhost -U postgres -D postgres supabase/tests/rls/*.test.sql supabase/tests/pairs/*.test.sql supabase/tests/rpc/*.test.sql supabase/tests/auth/*.test.sql
```
