...
| File                                            | Shows                                                                                                                                                                                                                                                             |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `01-rank-empty-import-{desktop,mobile}.png`     | `/rank` empty state: three equal affordances — `Rank My Rides` (board Mark Mode), `Sign up free` (join now, rank later), `Import a spreadsheet` — "no account needed".                                                                                            |
| `02-rank-workbench-search-{desktop,mobile}.png` | The `/rank` workbench with a seeded 3-coaster guest list: the shared add bar (search + `Add from park` + `Import list`, identical to `/me`), trimmed banner, and the save-only footer (`+ Add More` / `Save Ranking & Join Board`).                               |
...
| `05-park-picker-board-{desktop,mobile}.png`     | Park bulk-add picker opened from the FilterBar's `Add coasters from a park` button (next to search, Mark Mode only — same slot as on /me//rank); the FilterBar itself is now a full-bleed band on mobile (no card chrome). Cedar Point expanded, operating rows default-checked. |
| `06-me-park-button-{desktop,mobile}.png`        | `/me` sticky bar: `Add from park` beside `Import list` (icon-only on mobile), rendered by the shared `AddCoasterBar`.                                                                                                                                              |
