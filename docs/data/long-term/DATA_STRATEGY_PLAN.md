...
**Goal:** the existing catalog has clean, correctly-formed names, is duplicate-free, and has no obviously stale `under_construction`/`unknown` records — verified against the one metric that matters: nothing famous is missing or duplicated.

**Key operational fact that shapes this whole track:** `user_rides` is currently empty — the site isn't live yet. That removes the main reason to be conservative about record-level changes. There's no live user data that could be silently orphaned by a merge, no bookmarked slugs to preserve, no risk in reshuffling IDs. Track A can therefore lean more automated and more aggressive than a post-launch cleanup ever could — this is a one-time structural advantage worth spending deliberately, because it disappears the moment real users start ranking coasters.

### A.1 — Minimal schema addition (ship first, small migration)

Only the columns needed to make review state honest and support the audit pass below — nothing from the full provenance layer yet:
...
### A.3 — Combined audit pass: name cleanup + duplicate detection

Name quality (truncated names, park name embedded in the coaster name, stray abbreviations) and duplicate detection are the same underlying problem — messy names are *why* trigram similarity misses real duplicates and flags false ones — so this is one pass with an LLM on both ends of a cheap deterministic middle step, not three separate efforts.

1. **LLM normalization pass** over all ~1,087 records (batched, structured output per record):
   ```json
   {
     "coaster_id": "...",
     "cleaned_name": "Fury 325",
     "issue": "park_name_embedded",   // or: truncated | abbreviation | none
     "confidence": 0.9
   }
   ```
   High-confidence cleanups can be applied directly given the empty-`user_rides` situation (see callout above); low-confidence ones get flagged `needs_review` with the reason recorded. This is also what makes step 2 effective — matching runs against cleaned names, not raw scraped ones.

2. **`pg_trgm` candidate-pair generation** on cleaned `(name, park_id)` — cheap, deterministic, produces the working set. Start threshold ~0.6, tune by inspecting the false-positive rate.

3. **LLM adjudication pass** over the candidate pairs: given both full records (name, park, manufacturer, opening date, stats), return a verdict (`duplicate` / `not_duplicate` / `needs_human`) with confidence and reasoning. This is the same "recommender, not auto-merge" principle Track B's §3.6 formalizes for the long term — piloting it here, on a small and human-checkable dataset, is exactly the ground-truth run that pilot needs.

4. **Human review** only for the LLM's `needs_human` or low-confidence calls. Given the volume here (low hundreds of candidate pairs, at most), this is realistically a single working session, not something gated by the ~50/day ongoing-review capacity figure — that cap is about sustainable ongoing candidate throughput, not a one-time backlog.

5. **Apply merges.** Because `user_rides` is empty, there's no FK re-pointing to worry about — delete the duplicate outright, `on delete cascade` handles cleanup automatically. Still log every merge (cheap, and doubles as Track B's §3.6 pilot data):
...
### A.5 — Completeness pass: borrowed checklists first, ad-hoc discovery second

A hand-built "golden set" of must-have coasters is close to competitive work with the completeness pass itself — skip building one. Instead, borrow completeness checks that already exist:

1. **Published enthusiast/industry lists as a checklist, not a source.** Amusement Today's Golden Ticket Awards (running annually since 1998) publish Top Steel / Top Wooden Coaster rankings — a credible, already-curated ~50–75-name list. Cross-referencing those names + parks against the catalog is a 30–60 minute task and directly targets the coasters most likely to be checked by launch reviewers or enthusiast press. Only names/parks are compared — no content is copied. Note this source skews American; pair it with a non-US check (a Wikidata query for notable roller coasters by country, or a European equivalent list) so the checklist isn't geographically biased.
2. **RCDB mirrors as an existence lead, per the locked-in RCDB policy** — same "leads only, verify independently" rule as Track B's discovery campaigns (§3 of the original proposal, comment #3/#12). Fine to use here for the same reason: checking a name against a list, not importing data from it.
3. **Then** the broader ad-hoc pass: Wikidata/Wikipedia lists for major countries and top parks (Campaign A) — spot-check against the catalog, add anything obviously missing directly as `admin`-sourced coaster rows.
4. Skip: OSM (legal review pending, comment #13), government registries, manufacturer scraping, news monitoring — all Track B.

Stop condition is still qualitative, not a count — but running the borrowed-checklist step first means the stopping point is anchored to an external, independently-curated list rather than only your own sense of what's famous.

### What Track A explicitly does *not* build

Full provenance tables, candidate staging, source registry, confidence-scoring model, automated extraction, scheduled refresh jobs. All real, all deferred to Track B — building them now would spend the short launch-prep window on infrastructure instead of the data problem it's meant to solve.
