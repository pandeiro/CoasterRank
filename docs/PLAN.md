...
- **`v_coaster_rankings`** (view): the coaster columns the SPA reads (`id`, `park_id`, `name`, `slug`, `manufacturer_id`, `model`, `opening_date`, `status`, `material`, dimensions, `type`) **plus denormalized display/filter fields** — `park_name`, `park_slug`, `park_country`, `park_city`, `manufacturer_name`, `aliases text[]` (from `coaster_aliases`) — and `score`, `comparisons`, `participants`, `first_place_votes`, `rank = row_number() over (order by score desc)`. Dropped from the old `coasters.*` dump: `source`, `external_id`, and the review-metadata columns (never read by the SPA from the view). **Not** filtered by status — the SPA defaults to operating-only (clean URL `/`) and opts into everything via the "Include non-operational" checkbox (`?status=all`). The SPA board reads this view and filters/searches entirely client-side; the coaster detail page also reads its park fields from the row — no reference-table joins needed on either anymore.
