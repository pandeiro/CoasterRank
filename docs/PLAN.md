...
- **`profiles`(id→auth.users PK, username UNIQUE, display_name, avatar_url, is_admin bool default false)**
  ...
  - `username` is the public-facing handle; it must match `^[a-z0-9_]{3,20}$` and is unique case-insensitively. `display_name` can change freely.
