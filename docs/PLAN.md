...
- **Worker security headers** (`lib/security-headers.ts`, unit-tested in `worker.test.ts`): `Strict-Transport-Security`, `X-Content-Type-Options`, `Referrer-Policy`, `X-Frame-Options: SAMEORIGIN`, `Cross-Origin-Opener-Policy` on every response (static passthrough included; CSP Report-Only only on HTML). CSP ships as **Report-Only with no `report-uri`** (no collector endpoint — violations surface in the dev console); promote to enforced only after manual console review across the main routes with zero violations. Deliberately unset: `Cross-Origin-Embedder-Policy` (breaks cross-origin avatars/fonts without CORP).
