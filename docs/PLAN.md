...
- **`v_coaster_rankings`** (view): the coaster columns the SPA reads (`id`, `park_id`, `name`, `slug`, `manufacturer_id`, `model`, `opening_date`, `status`, `material`, dimensions, `type`) **plus denormalized display/filter fields** — `park_name`, `park_slug`, `park_country`, `manufacturer_name`, `aliases text[]` (from `coaster_aliases`) — and `score`, `comparisons`, `participants`, `first_place_votes`, `rank = row_number() over (order by score desc)`. Dropped from the old `coasters.*` dump: `source`, `external_id`, and the review-metadata columns (never read by the SPA from the view). **Not** filtered by status — the SPA defaults to operating-only (clean URL `/`) and opts into everything via the "Include non-operational" checkbox (`?status=all`). The SPA board reads this view and filters/searches entirely client-side; no reference-table joins needed on the board anymore.
...
First-place votes are only shown once the data is meaningful and non-identifying: the column populates when **more than 30 users** have submitted a ranking, and only for the **top 10 coasters by #1 votes** (ties broken by board rank, ≥1 vote required). Both rules are scale-free. Cells show `votes (pct)` where pct = votes ÷ that coaster's `participants` (share of its rankers who put it first).
...
- **Phase 3.0 — Board table + filtering rework** ✅: view slimmed to the columns the SPA reads and **denormalized for the board** (park name/slug/country, manufacturer name, `aliases text[]`) so search matches coaster name + park + former names and filtering needs no reference joins (board no longer fetches parks/manufacturers/countries); `coaster_ratings.first_place_votes` + `first_place_counts()` / `ranked_user_count()` RPCs; recompute upserts first-place counts on the same 15-min cadence as scores. Table columns: **Rank / Coaster / Park / Material / #1 votes** (score, comparisons, participants columns removed — FewVotesBadge still uses `comparisons` from the row). First-place visibility gated (PLAN §4.4.1): >30 ranked users and top-10 by votes, shown as `votes (pct-of-its-rankers)`. Filters simplified: text filter (coaster/park/alias), material 3-state view (Everything / Wooden only / Steel only — hybrids included under steel), country dropdown with pinned "Most coasters" optgroup (United States first) + alphabetical rest, manufacturer dropdown, and a single "Include non-operational" checkbox replacing the status select (`?status=all` in the URL; legacy `?status=<specific>` links fall back to the operating-only default; `?park=` dropped — the text filter covers park).
