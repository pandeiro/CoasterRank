...
- **Phase 5 — "My Coasters"** ✅: search/autocomplete to add coasters from the catalog; `@dnd-kit` drag-sort with auto-save on drop; rank renumbering (gapless 1-indexed); remove from list; per-user view with ranked count summary; unranked items shown separately at the bottom.
