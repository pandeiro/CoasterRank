...
| Board meta + homepage rework (2026-09) | `/api/ranking` gains `last_recomputed_at` + `real_user_count` via a new `public_board_meta()` security-definer RPC (aggregate-only, anon-granted like `ranked_user_count()`; source: `cron_execution_logs` success row + profiles minus admins minus synthetic). Homepage: lockup scaled ~12%, TrackLine removed, status line always-rendered with skeletons (CLS slot `min-h-[60vh|65vh]`), gated user count (>50), `Live ●` popunder (hover shows / click pins; `last_recomputed_at` preferred), `How it works →` → `/about` stub, table rework (Country column dropped, GCI/CCI acronyms, rank circles + coral podium 1:1 from rider-share tokens, ~5% denser rows, heavier tabular score, both layouts CSS-gated). Open final calls (score accent, palette/density alternatives) demoed on the design board (`/design.html`, dev-only) — rider-share tokens are the shipped starts. | Worker holds only the anon key (profiles/cron logs are RLS-locked; `coaster_ratings.updated_at` is first-insert stale), so one aggregate RPC is the only clean source; UI rework tracked as the homepage punchlist (§1–§8). |
...
- **Rank movement indicators (↑↓)**: needs new data — a weekly rank-snapshot table fed by the recompute cadence (preferred over per-15-min deltas, which would flicker as data trickles in), surfaced as `↑2 this week` in the board table. Top candidate for the next pass after Phase 3.1.
