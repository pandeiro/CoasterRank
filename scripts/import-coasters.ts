...
function mapStatus(raw: string): CoastStatus {
  ...
  if (v.includes('closed') || v.includes('temporarily') || v.includes('maintenance'))
    return 'sbno';
  return 'unknown';
}
...
function summarize(
  m: ReturnType<typeof buildModel>,
  rows: RawRow[],
): void {
...
async function apply(
  ...
  try {
    ...
    const verifyPark = await client.query("select count(*)::int as n from public.parks where source = 'open-csv'");
    const verifyCoaster = await client.query("select count(*)::int as n from public.coasters where source = 'open-csv'");

    console.log('\nWritten this run:');
    console.log('  coasters inserted:', inserted);
    console.log('  coasters updated:', updated, '(skipped non-csv:', m.coasters.length - inserted - updated + ')');
