    ...
    fc.assert(
      fc.property(
        fc.record({
          coaster_id: fc.string({ minLength: 1 }),
          cleaned_name: fc.string({ minLength: 1 }),
          issue: fc.string({ minLength: 1 }).filter((s) => !["park_name_embedded", "truncated", "abbreviation", "none"].includes(s)),
          confidence: fc.double({ min: 0, max: 1, noNaN: true }),
          reasoning: fc.string({ maxLength: 200 }),
        }),
        (invalid) => {
  ...
  it("rejects objects with unknown verdict value", () => {
    fc.assert(
      fc.property(
        fc.record({
          pair_id: fc.string({ minLength: 1 }),
          verdict: fc.string({ minLength: 1 }).filter((s) => !["duplicate", "not_duplicate", "needs_human"].includes(s)),
          confidence: fc.double({ min: 0, max: 1, noNaN: true }),
          reasoning: fc.string({ maxLength: 200 }),
        }),
        (invalid) => {
      ...
      fc.property(
        ...
        (json) => {
          ...
          if (result.success) {
            ...
            expect(["park_name_embedded", "truncated", "abbreviation", "none"]).toContain(result.data.issue);
