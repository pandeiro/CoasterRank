...
const MINI_W = 1672 // 2x raster of the 836x398 screenshot
const MINI_H = 796
// k = 1x px growth per side (kernel = Disk:2k at 2x); clean levels only
const MINI_LEVELS = [0, 1, 2, 3]
...
async function buildMinis() {
  ...
  const page = await browser.newPage({ viewport: { width: MINI_W, height: MINI_H } })
  ...
  // viewBox: union bbox + pad (from the thickest level so nothing clips)
  const thickest = Object.values(minis[minis.length - 1]).map((d) => bboxOf(d))
  const u = {
    x0: Math.min(...thickest.map((b) => b.minX)) / 10,
    y0: Math.min(...thickest.map((b) => b.minY)) / 10,
    x1: Math.max(...thickest.map((b) => b.maxX)) / 10,
    y1: Math.max(...thickest.map((b) => b.maxY)) / 10,
  }
  const pad = 16
  const miniVB = `${(u.x0 - pad).toFixed(0)} ${(2 * (MINI_H / 2) - u.y1 - pad).toFixed(0)} ${(u.x1 - u.x0 + 2 * pad).toFixed(0)} ${(u.y1 - u.y0 + 2 * pad).toFixed(0)}`
  const miniW = u.x1 - u.x0 + 2 * pad
  const miniH = u.y1 - u.y0 + 2 * pad

  const miniSvg = (layers, colors) => {
    const body = [
      ...[layers.hill].flat().map((d) => `<path d="${d}" fill="${colors.hill}"/>`),
      ...[layers.heart].flat().map((d) => `<path d="${d}" fill="${colors.heart}"/>`),
      ...[layers.loop].flat().map((d) => `<path d="${d}" fill="${colors.loop}"/>`),
    ].join('')
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${miniVB}">
  <title>CoasterRank Heartline mini mark</title>
  <g transform="translate(0,${MINI_H}) scale(0.1,-0.1)">${body}</g>
</svg>
`
  }
...
`
  ...
  return { minis_light, minis_dark, badgeMini, miniVB, miniW, miniH }
}

const { minis_light, minis_dark, badgeMini, miniVB, miniW, miniH } = await buildMinis()
...
for (const f of fs_readdirSync(dest)) if (f.startsWith('heartline-mini')) rmSync(`${dest}/${f}`)
