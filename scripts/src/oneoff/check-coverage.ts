...
async function main() {
  ...
  const {
    rows: [{ count: parkCount }],
  } = await pool.query('SELECT count(*)::int as count FROM parks')
  const {
    rows: [{ count: coasterCount }],
  } = await pool.query('SELECT count(*)::int as count FROM coasters')
  ...
  lines.push(
    `  ✅ Coaster found (total):  ${found.length} / ${entries.length} (${((found.length / entries.length) * 100).toFixed(1)}%)`,
  )
  ...
  const issues = results.filter(
    (r) =>
      !r.result.parkId ||
      !r.result.coasterId ||
      r.result.coasterSim < 0.95 ||
      r.result.parkSim < 0.95,
  )
  ...
  lines.push('='.repeat(70))
  lines.push(
    `  ${exactCoaster.length} exact + ${fuzzyCoaster.length} fuzzy = ${found.length} matched out of ${entries.length}`,
  )
