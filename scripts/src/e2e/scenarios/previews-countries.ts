 ...
 *
 * Fixtures are tuned so every state shows: Germany leads on average (short
 * bench of 2), the US has a full six-ride bench (top-five cap in effect, B&M
 * the clear top builder), Japan sits mid-pack (short bench of 4), and France
 * brings a single ranked ride.
 */
...
const rows = [
  // Germany: (3 + 5) / 2 = 4.0 — leads, short bench.
  ...
  // Japan: (6 + 8 + 10 + 11) / 4 = 8.75 — short bench of 4.
  ...
  // France: a single ranked ride — the deepest short bench.
    ...
    files.push(
      ...(await capturePreviews(
        ...
        async (kind) => {
          ...
          await waitForVisible(page, 'text=avg 4.0')
          return page
        },
        { fullPage: true },
      )),
