...
/**
 * The subtle share nudge on My Coasters: "Want to share your board?" over a
 * flat, simplified take on the rider OG share card — a single accent-tinted
 * card with the avatar, display-font name, ranked/parks pills and the
 * copyable rider URL + copy/preview tucked beneath, and the OG card's top-3
 * rows (Racing Sans One ranks, #1 coral) directly on the tint. The
...
export default function ShareNudgeBanner({
  ...
  return (
    <Panel className="relative p-4 sm:p-5" data-testid="share-nudge-banner">
      ...
      {username ? (
        <div className="mt-3 flex flex-wrap gap-x-8 gap-y-4 rounded-xl border border-accent/30 bg-accent/10 p-4">
          <div className="flex min-w-0 flex-1 basis-72 flex-col justify-between gap-3">
            ...
            {url ? (
              ...
              <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
                <p className="min-w-0 flex-1 basis-64 text-sm leading-snug text-muted">
          ...
          {topThree.length > 0 && (
            <div className="min-w-0 flex-1 basis-72">
              ...
              <ul className="mt-1 divide-y divide-line">
                {topThree.map((ride) => (
                  <li key={ride.rank} className="flex items-center gap-3 py-2 first:pt-2">
                    <span
                      className={`display-heading w-7 text-center text-base leading-none ${
