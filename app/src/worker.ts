...
export interface Env {
  ...
  COASTER_RANK_EVENTS_BOT_TOKEN?: string
...
// ── /api/events: real-time signals for Telegram EVENTS ───────────────────
// Minimal-privacy: country only (ISO2 from Cloudflare edge), no IP stored.
// Used by the SPA fire-and-forget after signup / submission / share opt-in.
// Kill-switch: app_settings rows (signup_events / submission_events / share_events)
// — when disabled, Telegram is silenced but the row is still logged.

export type AnalyticsEventType = 'signup' | 'submission' | 'share'
const VALID_EVENT_TYPES: AnalyticsEventType[] = ['signup', 'submission', 'share']
const EVENT_SETTING_KEY: Record<AnalyticsEventType, string> = {
  signup: 'signup_events',
...
export function getCountryFromRequest(request: Request): string | null {
  ...
  if (typeof cfCountry === 'string' && /^[A-Za-z]{2}$/.test(cfCountry))
    return cfCountry.toUpperCase()
  const header =
    request.headers.get('CF-IPCountry') ??
    request.headers.get('cf-ipcountry') ??
    request.headers.get('cf-ip-country')
  if (typeof header === 'string' && /^[A-Za-z]{2}$/.test(header.trim()))
    return header.trim().toUpperCase()
  return null
}

function isSyntheticPayload(payload: Record<string, unknown>): boolean {
  const email =
    (payload.email as string) ?? (payload.meta as Record<string, unknown> | undefined)?.email
  if (typeof email === 'string' && email.toLowerCase().endsWith('@test.coasterrank.dev'))
    return true
  const rawSynthetic = (payload.meta as Record<string, unknown> | undefined)?.synthetic
  if (rawSynthetic === true) return true
  return false
}

async function isEventEnabled(settingKey: string, env: Env): Promise<boolean> {
  const supabaseUrl = env.SUPABASE_URL ?? env.VITE_SUPABASE_URL
  const serviceKey = env.SUPABASE_SERVICE_ROLE_KEY
  if (!supabaseUrl || !serviceKey) return true
  try {
    const res = await fetch(
      `${supabaseUrl}/rest/v1/app_settings?key=eq.${encodeURIComponent(settingKey)}&select=enabled`,
      {
...
async function insertAnalyticsEvent(
  ...
  try {
    ...
    // Best-effort: logging failure must not break the client.
  }
}
...
async function sendEventTelegram(
  ...
  const botToken = env.COASTER_RANK_EVENTS_BOT_TOKEN
  ...
  let text: string
  if (type === 'signup') {
    text = `[${appEnv}] ✨ New user: @${username ?? 'unknown'}${country ? ` — ${country}` : ''}`
  } else if (type === 'submission') {
    const coaster =
      (meta.coaster_name as string) ?? (payload.coaster_name as string) ?? 'unknown coaster'
    const park = (meta.park_name as string) ?? (payload.park_name as string) ?? 'unknown park'
    text = `[${appEnv}] 📝 Submission: "${coaster}" @ ${park} — by @${username ?? 'unknown'}`
  } else {
    const count = (meta.rankedCount as number) ?? (payload.rankedCount as number)
    text = `[${appEnv}] 📣 @${username ?? 'unknown'} shared their list${typeof count === 'number' ? ` — ${count} ranked` : ''}`
  }
  try {
...
export async function handleEventsRequest(request: Request, env: Env): Promise<Response> {
  const origin = request.headers.get('origin')
  const cors = origin ? corsHeaders(request, env) : {}
  ...
  const settingKey = EVENT_SETTING_KEY[type]
  const [enabled] = await Promise.all([isEventEnabled(settingKey, env)])
  // Always log; Telegram only when enabled and not synthetic.
  const logPayload = { type, username, country, meta }
  await insertAnalyticsEvent(logPayload, env)

  const synthetic = isSyntheticPayload(body as Record<string, unknown>)
  if (enabled && !synthetic) {
    // Fire Telegram but don't await failure blocks — run in background yet awaited for test determinism.
    await sendEventTelegram(type, { username, country, meta, ...body }, env)
  }
